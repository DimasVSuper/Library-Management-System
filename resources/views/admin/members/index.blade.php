...
@section('content')
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('members.create') }}" class="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-3 rounded-lg font-semibold transition duration-300 transform hover:scale-105">
            + Tambah Member
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden">
            ...
            <tbody>
                ...
                <tr class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition">
                    <td class="px-6 py-4 text-slate-900 dark:text-white font-medium">
                        <a href="{{ route('members.show', $member->id) }}" class="text-blue-600 dark:text-blue-400 hover:underline">
                            {{ $member->name }}
                        </a>
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex gap-2 justify-center">
                            <a href="{{ route('members.edit', $member->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm font-semibold transition">
                                Edit
                            </a>
                            <form action="{{ route('members.destroy', $member->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="text-center py-12">
            <p class="text-slate-600 dark:text-slate-400 text-lg">Belum ada member. <a href="{{ route('members.create') }}" class="text-blue-600 dark:text-blue-400 hover:underline font-semibold">Tambah member sekarang</a></p>
        </div>
        @endif
    </div>
@endsection
